...
void proccessingFun(double buffer[][BLOCK_SIZE],all_gains* gains)
{
	...
	//tremolo function
		double* buffptr=&buffer[3][0];
		double* buffptr1=&buffer[4][0];
		...
		if(selected==2)
		{
				...
				for(i=0;i<BLOCK_SIZE;i++)
				{
					buffer[3][i]=buffer[0][i];
					buffer[4][i]=buffer[1][i];
				}

				processBlock(&buffer[3][0],&buffer[3][0],&tremolo_data,BLOCK_SIZE);
				processBlock(&buffer[4][0],&buffer[4][0],&tremolo_data,BLOCK_SIZE);
		}
		...
		for(i=0;i<BLOCK_SIZE;i++)
		{
			...
			//if(selected==2)
				//buffer[2][i]+=(buffer[2][i]*gains->input_gain);
